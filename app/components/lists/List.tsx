...
// Component
const List = ({ list, formId, onEdit }: ListProps) => {
  // Hooks
  const navigate = useNavigate();
  const listDispatch = useListDispatch();

  // Derived state
  const { id, name, items, slug } = list;
      ...
      <ul className="mt-4 grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-4 sm:gap-x-6 lg:grid-cols-8 lg:gap-x-8 xl:gap-x-12">
        <li>
          <div className="aspect-2/3">
            <button
              type="button"
              className="group h-full w-full overflow-hidden rounded-lg border-2 border-dashed border-gray-300 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
